...
export const buildWeeklyExportModel = async ({ weeklyRepository, subjectId, weeklyReportId, now=()=>Date.now() }) => {
  const value=await weeklyRepository.exportModel(subjectId,weeklyReportId);
  if(!value)throw new WorkLogError("WEEKLY_REPORT_NOT_FOUND",404);
  const {report,source}=value;
  return {format:"jilu-weekly-report-export",schemaVersion:1,exportedAt:new Date(now()).toISOString(),reportType:"WEEKLY",
    weekStart:report.weekStart,weekEnd:report.weekEnd,title:report.title,content:report.content,entries:report.entries,status:report.status,
    sourceChanged:report.sourceChanged,sourceDigest:report.sourceDigest,contentDigest:report.contentDigest,aggregatorVersion:report.aggregatorVersion,
    realizerVersion:report.realizerVersion,generatedAt:report.generatedAt,createdAt:report.createdAt,updatedAt:report.updatedAt,
    sourceDayCount:new Set(source.map(x=>x.localDate)).size,sourceItemCount:source.length,
    sourceItems:source.map((x,index)=>({date:x.localDate,order:index+1,presentation:x.presentation,project:x.projectName||"",location:x.location||"",sourceType:x.userEdited?"人工编辑":"自动生成",dailyStatus:x.status||""}))};
};
export const renderWeeklyExportJson = model => new TextEncoder().encode(JSON.stringify(model,null,2));
...
export class ExportService {
  /** @param {any} options */
  constructor({ repository, weeklyRepository=null, jobs = new MemoryExportRepository(), artifacts = new MemoryArtifactStore(), now = () => Date.now(), retentionMs = EXPORT_RETENTION_MS, id = () => `exp_${crypto.randomUUID().replace(/-/g, "")}` }) { this.repository = repository; this.weeklyRepository=weeklyRepository; this.jobs = jobs; this.artifacts = artifacts; this.now = now; this.retentionMs = retentionMs; this.id = id; }
  async create(subjectId, input) { const format = String(input?.format || "").toUpperCase(),reportType=String(input?.reportType||"DAILY").toUpperCase(); if (!new Set(["JSON", "XLSX"]).has(format)) throw new WorkLogError("EXPORT_FORMAT_UNSUPPORTED", 400); if(!new Set(["DAILY","WEEKLY"]).has(reportType))throw new WorkLogError("EXPORT_INVALID_QUERY",400); if (!input?.clientExportId || String(input.clientExportId).length > 160) throw new WorkLogError("EXPORT_INVALID_QUERY", 400); const weeklyReportId=reportType==="WEEKLY"?String(input.weeklyReportId||""):null;if(reportType==="WEEKLY"&&(!weeklyReportId||!this.weeklyRepository))throw new WorkLogError("EXPORT_INVALID_QUERY",400);const query = reportType==="WEEKLY"?{reportType,weeklyReportId}:validateExportQuery(input.query), queryDigest = await payloadDigest({ format, query }), createdAt = this.now(), exportId = this.id(); let job = await this.jobs.create({ exportId, subjectId, clientExportId: String(input.clientExportId), queryDigest, format, status: "PENDING", query, createdAt, startedAt: null, readyAt: null, expiresAt: null, filename: null, contentType: null, sizeBytes: null, recordCount: null, errorCode: null, artifactKey: null }); if (job.exportId !== exportId) return publicJob(job); try { job = await this.jobs.update(subjectId, exportId, { status: "RUNNING", startedAt: this.now() }); const model = reportType==="WEEKLY"?await buildWeeklyExportModel({weeklyRepository:this.weeklyRepository,subjectId,weeklyReportId,now:this.now}):await buildExportModel({ repository: this.repository, subjectId, query, now: this.now }); const bytes = format === "JSON" ? (reportType==="WEEKLY"?renderWeeklyExportJson(model):renderExportJson(model)) : (reportType==="WEEKLY"?renderWeeklyExportXlsx(model):renderExportXlsx(model)), ext = format.toLowerCase(), sameDay=query.dateFrom===query.dateTo, filename = reportType==="WEEKLY"?`工作周报_${model.weekStart}_至_${model.weekEnd}.${ext}`:format==="XLSX" ? `工作日报_${query.dateFrom}${sameDay?"":`_至_${query.dateTo}`}.xlsx` : `工作日志_${query.dateFrom}${sameDay?"":`_至_${query.dateTo}`}.json`, artifactKey = `${subjectId.replace(/[^A-Za-z0-9_-]/g, "_")}/${exportId}.${ext}`; await this.artifacts.put(artifactKey, bytes); job = await this.jobs.update(subjectId, exportId, { status: "READY", artifactKey, filename, contentType: format === "JSON" ? "application/json; charset=utf-8" : "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", sizeBytes: bytes.length, recordCount: reportType==="WEEKLY"?model.sourceItemCount:model.logs.length + model.items.length, readyAt: this.now(), expiresAt: this.now() + this.retentionMs }); } catch (error) { const failure = /** @type {any} */ (error); job = await this.jobs.update(subjectId, exportId, { status: "FAILED", errorCode: failure.code || "EXPORT_FAILED" }); } return publicJob(job); }
