...
export const planWorkLogItem = (source = {}) => {
  ...
  if (natural) {type=result?"RESULT_ITEM":"ACTION_ITEM";title=shortObject(issue&&action?`${object||""}故障处理`:(object?`${object}${action||"记录"}`:clean(source.category)||action));content=natural;}
