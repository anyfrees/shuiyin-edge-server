...
const ACTIONS = ["巡检","检查","核对","盘点","录入","配置","调试","排查","处理","维修","更换","安装","部署","办理","保障","测试","登记","整理","归档"];
const ISSUES = ["故障","异常","离线","无信号","黑屏","断网","中断","卡顿","损坏","报错","告警"];
const RESULTS = ["恢复正常","处理完成","恢复使用","已解决","已更换","已完成","验收通过"];
...
export const normalizeAtomicWorkFacts = (source = {}) => {
  ...
  const results = unique(source.results || [source.result, ...RESULTS.filter((word) => raw.includes(word))]).filter((x) => !negatives.some((negative) => negative.includes(x) || x.includes(negative)));
...
export const planWorkLogItem = (source = {}) => {
  ...
  if (natural) {type=result?"RESULT_ITEM":"ACTION_ITEM";title=shortObject(clean(source.category)||`${object||""}${action||""}`);content=natural;}
  ...
  else if (issue) { type="ISSUE_ONLY_ITEM"; title=shortObject(clean(source.category)||issue); const issueText=object&&issue.includes(object)?issue:`${object||""}${issue}`;content=`记录${locations||""}${issueText}情况`; }
  ...
  if (purpose) content+=`，${purpose}`;
  if (result) { type="RESULT_ITEM"; content+=`，${result}`; }
  if (negative) content+=`，${negative}`;
...
export const semanticRuleBasedDraft = (capture) => { const item=planWorkLogItem({...capture,category:sourceCategory(capture)}); return {logTitle:`${capture.localDate||capture.local_date} 工作记录`,summary:`今日${clause(item)}。`,category:sourceCategory(capture),itemTitle:item.title,content:item.content,result:item.facts.results[0]||"",note:"",plannerVersion:item.plannerVersion,origin:item.origin}; };
