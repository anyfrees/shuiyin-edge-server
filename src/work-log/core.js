...
export const validateCaptureSnapshot = snapshot => {
  ...
  strictKeys(snapshot,["schemaVersion","state","capture","template","project","location","weather","fields","rendered","photo","provenance"]);strictKeys(c,["clientCaptureId","jiluCode","sourceType","capturedAt","captureRequestedAt","captureCompletedAt","timezone","utcOffsetMinutes"]);strictKeys(snapshot.template,["origin","templateId","customTemplateId","builtinId","version","nameSnapshot"]);strictKeys(snapshot.project,["projectId","projectNameSnapshot","projectMatchSource","projectMatchRuleId","projectMatchRuleVersion","projectMatchedAt"]);strictKeys(snapshot.location,["source","name","address","latitude","longitude","accuracyMeters","altitudeMeters"]);if(snapshot.weather)strictKeys(snapshot.weather,["textSnapshot"]);strictKeys(snapshot.rendered,["textSnapshot","lines"]);strictKeys(p,["sha256","storageState"]);strictKeys(snapshot.provenance,["clientTaskId","recordId"]);
  const project=snapshot.project||{},matchSource=project.projectMatchSource||(project.projectId?"DEFAULT_PROJECT":"NONE");
  if(!["LOCATION_GEOFENCE","DEFAULT_PROJECT","NONE"].includes(matchSource))throw new WorkLogError("PROJECT_MATCH_SOURCE_INVALID");
  if(matchSource==="NONE"&&(project.projectId||project.projectNameSnapshot||project.projectMatchRuleId))throw new WorkLogError("PROJECT_MATCH_INVALID");
  if(matchSource!=="NONE"&&(!project.projectId||!project.projectNameSnapshot))throw new WorkLogError("PROJECT_MATCH_INVALID");
  bounded(project.projectMatchRuleId,200,"PROJECT_MATCH_INVALID");
