...
var TemplateRuntimeService = class {
  ...
  async authorized(subject, templateId, templateVersion) {
    ...
    if (!access.allowed || !version || version.status !== "PUBLISHED" || version.templateVersion !== Number(templateVersion))
      throw unavailable();
    const templateEpoch = await this.repository.getEpoch(templateId), subjectEpoch = await this.repository.getEpoch(`subject:${subject.subjectId}`);
    return {
      ctx,
      access,
      version,
...
var TemplatePublishService = class {
  ...
  async prepare({ templateId, templateVersion, actorId = "admin" }) {
    ...
    this.validateTemplate(template);
    const key2 = this.activeKey(), draft = version.draft || {}, layoutBytes = enc2.encode(canonical(draft.layout)), files = { "layout.json": b64(layoutBytes) }, entries = [];
    for (const asset of [...(draft.assets || [])].sort((a, b) => a.path.localeCompare(b.path))) {
      const assetBytes = bytes(asset.bytes ?? asset.data);
      files[asset.path] = b64(assetBytes);
      entries.push({ id: asset.id, path: asset.path, sha256: await sha(assetBytes), mimeType: asset.mimeType, size: assetBytes.length });
    }
