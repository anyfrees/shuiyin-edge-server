...
export const createEdgeAdminHandler = ({ kv, env, forward, forwardToken, backupStorage, waitUntil }) => async request => {
  ...
  try {
    ...
    if(path==='/notifications/summary'&&method==='GET'){await service.require(access,{permission:'template.read'});const submissionNames=await listNames(kv,'submission:record:'),inquiryNames=await listNames(kv,'creator:inquiry:');let pendingSubmissions=0,openInquiries=0;for(const name of submissionNames)if(parse(await kv.get(name))?.status==='PENDING')pendingSubmissions++;for(const name of inquiryNames)if(parse(await kv.get(name))?.status==='OPEN')openInquiries++;return json({ok:true,pendingSubmissions,openInquiries})}
    const inquiryAction=path.match(/^\/inquiries\/([^/]+)\/resolve$/);if(inquiryAction&&method==='POST'){await service.require(access,{permission:'template.read'});const name=`creator:inquiry:${decodeURIComponent(inquiryAction[1])}`,record=parse(await kv.get(name));if(!record||record.status!=='OPEN')throw fail('INQUIRY_NOT_FOUND_OR_RESOLVED',409);record.status='RESOLVED';record.resolvedAt=Date.now();record.resolvedBy=access.principal.adminId;record.reply=clean((await bodyOf(request)).reply||'已处理',1000);await kv.put(name,JSON.stringify(record));await service.audit(access.principal.adminId,'CREATOR_INQUIRY_RESOLVE','inquiry',record.inquiryId);return json({ok:true})}
    ...
    if(submissionAction&&method==='POST'){await service.require(access,{permission:'template.publish'});const record=parse(await kv.get(`submission:record:${decodeURIComponent(submissionAction[1])}`));if(!record||record.status!=='PENDING')throw fail('SUBMISSION_NOT_FOUND_OR_REVIEWED',409);const input=await bodyOf(request),approve=submissionAction[2]==='approve';if(approve&&!clean(input.templateId,96))throw fail('INVALID_TEMPLATE_ID',400);record.status=approve?'APPROVED':'REJECTED';record.reviewedAt=Date.now();record.reviewedBy=access.principal.adminId;record.reviewNote=clean(input.reviewNote||(approve?'审核通过并发布':'未通过本次审核'),300);record.publishedTemplateId=approve?clean(input.templateId,96):null;await clearSubmissionPackage(record);await kv.put(`submission:record:${record.submissionId}`,JSON.stringify(record));await service.audit(access.principal.adminId,approve?'TEMPLATE_SUBMISSION_APPROVE':'TEMPLATE_SUBMISSION_REJECT','submission',record.submissionId,'SUCCESS',{templateId:record.publishedTemplateId});return json({ok:true,status:record.status,templateId:record.publishedTemplateId,packageDeleted:true})}
