...
export const createEdgeAdminHandler = ({ kv, env, forward, forwardToken, backupStorage, waitUntil, identities }) => async request => {
  ...
  try {
    ...
    if (path === '/auth/wechat-code' && method === 'POST') { const input=await bodyOf(request),session=await identities.withBindingCode(input.bindingCode,async subjectId=>{const adminId=await kv.get(key('wechat-subject',subjectId));if(!adminId)throw fail('ADMIN_WECHAT_NOT_BOUND',403);const admin=await service.principal(adminId);if(!admin||admin.status!=='ACTIVE')throw fail('ADMIN_DISABLED',401);await service.audit(adminId,'ADMIN_WECHAT_LOGIN','admin',adminId);return service.issueSession(admin,'WECHAT_CODE')});return json({ok:true,csrfToken:session.csrf,expiresAt:session.expiresAt,method:session.method},200,{'Set-Cookie':sessionCookie(session,env.ENVIRONMENT!=='test')}) }
    if(path==='/auth/qr/options'&&method==='POST'){const requestId=`qr_${random(18)}`,secret=random(24),expiresAt=Date.now()+120000,record={requestId,secretHash:await digest(secret),purpose:'ADMIN',status:'PENDING',createdAt:Date.now(),expiresAt};await kv.put(`web-qr:${requestId}`,JSON.stringify(record),{expirationTtl:180});return json({ok:true,requestId,secret,purpose:'admin',expiresAt,qrContent:`https://shuiyin.nnu.cn/wx-login?request=${encodeURIComponent(requestId)}&secret=${encodeURIComponent(secret)}&purpose=admin`},201)}
    if(path==='/auth/qr/verify'&&method==='POST'){const input=await bodyOf(request),name=`web-qr:${clean(input.requestId,96)}`,record=parse(await kv.get(name));if(!record||record.secretHash!==await digest(input.secret)||record.purpose!=='ADMIN'||record.expiresAt<=Date.now()||record.usedAt)throw fail('QR_LOGIN_INVALID',404);if(record.status==='PENDING')return json({ok:true,pending:true,expiresAt:record.expiresAt},202);record.usedAt=Date.now();await kv.put(name,JSON.stringify(record),{expirationTtl:60});const adminId=await kv.get(key('wechat-subject',record.subjectId));if(!adminId)throw fail('ADMIN_WECHAT_NOT_BOUND',403);const admin=await service.principal(adminId);if(!admin||admin.status!=='ACTIVE')throw fail('ADMIN_DISABLED',401);const session=await service.issueSession(admin,'WECHAT_QR');await service.audit(adminId,'ADMIN_WECHAT_QR_LOGIN','admin',adminId);return json({ok:true,csrfToken:session.csrf,expiresAt:session.expiresAt,method:session.method},200,{'Set-Cookie':sessionCookie(session,env.ENVIRONMENT!=='test')})}
