...
export const createEdgeAdminHandler = ({ kv, env, forward, forwardToken, backupStorage, waitUntil, identities }) => async request => {
  ...
  try {
    ...
    const projectListMatch=path.match(/^\/subjects\/([^/]+)\/projects$/),geofenceMatch=path.match(/^\/subjects\/([^/]+)\/projects\/([^/]+)\/geofence$/)
    if(projectListMatch&&method==='GET'){await service.require(access,{permission:'template.read'});const publicId=decodeURIComponent(projectListMatch[1]),subjectId=await kv.get(`public_${publicId}`);if(!subjectId)throw fail('SUBJECT_NOT_FOUND',404);const repository=env.PROVENANCE_D1?new D1WorkLogRepository(env.PROVENANCE_D1):env.PROVENANCE_BLOB?new EdgeOneBlobWorkLogRepository(env.PROVENANCE_BLOB):null;if(!repository)throw fail('WORK_LOG_STORAGE_NOT_CONFIGURED',503);const projects=await repository.listProjects(subjectId),items=await Promise.all(projects.map(async project=>({...project,geofence:await repository.getProjectGeofence(subjectId,project.project_id||project.projectId)})));return json({ok:true,items})}
    if(geofenceMatch&&method==='PUT'){await service.require(access,{permission:'grant.user'});const publicId=decodeURIComponent(geofenceMatch[1]),projectId=decodeURIComponent(geofenceMatch[2]),subjectId=await kv.get(`public_${publicId}`);if(!subjectId)throw fail('SUBJECT_NOT_FOUND',404);const repository=env.PROVENANCE_D1?new D1WorkLogRepository(env.PROVENANCE_D1):env.PROVENANCE_BLOB?new EdgeOneBlobWorkLogRepository(env.PROVENANCE_BLOB):null;if(!repository)throw fail('WORK_LOG_STORAGE_NOT_CONFIGURED',503);const oldRule=await repository.getProjectGeofence(subjectId,projectId),rule=await repository.upsertProjectGeofence(subjectId,projectId,await bodyOf(request)),requestId=clean(request.headers.get('x-request-id'),120)||`req_${random(12)}`;await service.audit(access.principal.adminId,'PROJECT_GEOFENCE_UPDATE','project',projectId,'SUCCESS',{publicId,oldRule,newRule:rule,requestId});return json({ok:true,rule})}
