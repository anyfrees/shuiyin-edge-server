...
const coseKey = async cose => { const map=cbor(cose).value,kty=map.get(1),alg=map.get(3);if(kty===2&&alg===-7){const x=map.get(-2),y=map.get(-3);return crypto.subtle.importKey('raw',concat(Uint8Array.of(4),x,y),{name:'ECDSA',namedCurve:'P-256'},false,['verify'])}if(kty===3&&alg===-257)return crypto.subtle.importKey('jwk',{kty:'RSA',n:b64u(map.get(-1)),e:b64u(map.get(-2)),alg:'RS256',ext:true},{name:'RSASSA-PKCS1-v1_5',hash:'SHA-256'},false,['verify']);throw fail('PASSKEY_ALGORITHM_UNSUPPORTED',400) }
...
export class EdgeAdminSecurityService {
  ...
  async verifyAuthentication(response) {
    ...
    const challenge = await this.consumeChallenge(admin.adminId, 'AUTHENTICATE'),clientRaw=await verifyClient(response,challenge.challenge,this.origins,'webauthn.get'),authData=fromB64u(response.response.authenticatorData);if(!await rpHashValid(authData,this.rpId)||(authData[32]&5)!==5)throw fail('ADMIN_LOGIN_FAILED',401);const counter=new DataView(authData.buffer,authData.byteOffset+33,4).getUint32(0),signed=concat(authData,new Uint8Array(await crypto.subtle.digest('SHA-256',clientRaw))),publicKey=await coseKey(fromB64u(passkey.publicKey)),algorithm=publicKey.algorithm.name==='ECDSA'?{name:'ECDSA',hash:'SHA-256'}:{name:'RSASSA-PKCS1-v1_5'},signature=fromB64u(response.response.signature),verified=await crypto.subtle.verify(algorithm,publicKey,publicKey.algorithm.name==='ECDSA'?derToRaw(signature):signature,signed);if(!verified||(passkey.counter&&counter<=passkey.counter))throw fail('ADMIN_LOGIN_FAILED',401);passkey.counter=counter; passkey.lastUsedAt = this.now(); await put(this.kv, 'passkey', passkey.credentialId, passkey); await this.audit(admin.adminId, 'ADMIN_LOGIN_SUCCESS', '', '', 'SUCCESS', { method: 'PASSKEY' }); return this.issueSession(admin, 'PASSKEY')
  }
