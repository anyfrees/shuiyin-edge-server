...
export class EdgeAdminSecurityService {
  ...
  async verifyAuthentication(response) {
    ...
    const challenge = await this.consumeChallenge(admin.adminId, 'AUTHENTICATE'),clientRaw=await verifyClient(response,challenge.challenge,this.origins,'webauthn.get'),authData=fromB64u(response.response.authenticatorData);if(!await rpHashValid(authData,this.rpId)||(authData[32]&5)!==5)throw fail('ADMIN_LOGIN_FAILED',401);const counter=new DataView(authData.buffer,authData.byteOffset+33,4).getUint32(0),signed=concat(authData,new Uint8Array(await crypto.subtle.digest('SHA-256',clientRaw))),material=await coseKey(fromB64u(passkey.publicKey)),signature=fromB64u(response.response.signature),verified=await crypto.subtle.verify(material.verifyAlgorithm,material.key,material.ecdsa?derToRaw(signature):signature,signed);if(!verified||(passkey.counter&&counter<=passkey.counter))throw fail('ADMIN_LOGIN_FAILED',401);passkey.counter=counter; passkey.lastUsedAt = this.now(); await put(this.kv, 'passkey', passkey.credentialId, passkey); const session=await this.issueSession(admin, 'PASSKEY'); try{await this.audit(admin.adminId, 'ADMIN_LOGIN_SUCCESS', '', '', 'SUCCESS', { method: 'PASSKEY' })}catch{} return session
  }
