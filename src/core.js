...
export async function handleRequest(request, env, kv) {
  ...
  if(url.pathname==="/v1/creator/inquiries"&&request.method==="POST"){if(!kv)return json({ok:false,code:"PERSISTENT_STORAGE_NOT_CONFIGURED"},503,headers);const subjectId=await creatorSessionSubject();if(!subjectId)return json({ok:false,code:"CREATOR_SESSION_INVALID"},401,headers);const input=await request.json(),subject=cleanText(input.subject,80),content=cleanText(input.content,1000);if(!subject||!content)return json({ok:false,code:"INQUIRY_INVALID"},400,headers);const inquiryId=`inq_${crypto.randomUUID().replaceAll("-","")}`,now=Date.now(),record={inquiryId,subjectId,subject,content,status:"OPEN",createdAt:now,updatedAt:now,resolvedAt:null,reply:"",messages:[{messageId:`im_${crypto.randomUUID().replaceAll("-","")}`,senderType:"USER",content,createdAt:now}]};await kv.put(`creator:inquiry:${inquiryId}`,JSON.stringify(record));return json({ok:true,inquiryId},201,headers)}
  const creatorInquiryMessage=url.pathname.match(/^\/v1\/creator\/inquiries\/([^/]+)\/messages$/);if(creatorInquiryMessage&&request.method==="POST"){if(!kv)return json({ok:false,code:"PERSISTENT_STORAGE_NOT_CONFIGURED"},503,headers);const subjectId=await creatorSessionSubject();if(!subjectId)return json({ok:false,code:"CREATOR_SESSION_INVALID"},401,headers);const name=`creator:inquiry:${decodeURIComponent(creatorInquiryMessage[1])}`,record=JSON.parse(await kv.get(name)||"null"),input=await request.json(),content=cleanText(input.content,1000);if(!record||record.subjectId!==subjectId)return json({ok:false,code:"INQUIRY_NOT_FOUND"},404,headers);if(!content)return json({ok:false,code:"INQUIRY_INVALID"},400,headers);const now=Date.now();record.messages=Array.isArray(record.messages)?record.messages:[];record.messages.push({messageId:`im_${crypto.randomUUID().replaceAll("-","")}`,senderType:"USER",content,createdAt:now});record.status="OPEN";record.updatedAt=now;record.resolvedAt=null;await kv.put(name,JSON.stringify(record));return json({ok:true,status:"OPEN"},201,headers)}
