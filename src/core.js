...
import { D1WeeklyReportRepository, EdgeOneWeeklyReportRepository } from "./work-log/weekly-report-repositories.js";
...
export async function handleRequest(request, env, kv) {
  ...
  if (/^\/v1\/(?:captures|work-logs|projects|project-match-rules|tags|exports)(?:\/|$)/.test(url.pathname)) {
    ...
    const service = new WorkLogHttpService({repository,enabled:workLogEnabled(env),autoDraftService,exportService,exportEnabled,weeklyRepository,weeklyEnabled,cursorSecret:env.JILU_SUBJECT_DERIVATION_KEY||"work-log-v1-local",authorize:async(subjectId,capability)=>Boolean(subjectEntitlements&&await subjectEntitlements.isGranted(subjectId,capability)),authenticate:async req=>{const token=bearer(req);try{const auth=await authService(env,kv).authenticate(token);return{subjectId:auth.subject.subjectId,authType:"MINI"}}catch(error){if(!kv)throw error;const hash=await sha256(token),session=JSON.parse(await kv.get(`creator:session:${hash}`)||"null");if(session?.subjectId&&session.expiresAt>Date.now())return{subjectId:session.subjectId,authType:"CREATOR"};throw error}},verifyProvenanceOwnership:async(subjectId,recordId)=>{if(env.PROVENANCE_D1)return Boolean(await env.PROVENANCE_D1.prepare("SELECT 1 ok FROM provenance_records WHERE subject_id=? AND record_id=?").bind(subjectId,recordId).first());if(env.PROVENANCE_BLOB){const record=await new EdgeOneBlobProvenanceCommitRepository(env.PROVENANCE_BLOB).getRecordById(recordId);return record?.subjectId===subjectId}return false}}),result=await service.handle(request),merged=new Headers(result.headers);Object.entries(headers).forEach(([key,value])=>merged.set(key,value));return new Response(result.body,{status:result.status,headers:merged});
  }
