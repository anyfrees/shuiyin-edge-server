...
export async function handleRequest(request, env, kv) {
  ...
  const cleanupCreatorFeedback=async()=>{const cutoff=Date.now()-30*86400000,names=(await kv.list({prefix:"creator:inquiry:"})).keys||[];for(const item of names){const name=item.name||item.key,record=JSON.parse(await kv.get(name)||"null");if(record&&Number(record.updatedAt||record.createdAt)<cutoff)await kv.delete(name)}};
  ...
  if(url.pathname==="/v1/creator/me"&&request.method==="GET"){if(!kv)return json({ok:false,code:"PERSISTENT_STORAGE_NOT_CONFIGURED"},503,headers);const subjectId=await creatorSessionSubject();if(!subjectId)return json({ok:false,code:"CREATOR_SESSION_INVALID"},401,headers);await cleanupCreatorFeedback();const subject=JSON.parse(await kv.get(`subject_${subjectId}`)||"null"),submissionNames=(await kv.list({prefix:"submission:record:"})).keys||[],inquiryNames=(await kv.list({prefix:"creator:inquiry:"})).keys||[],submissions=[],inquiries=[];for(const item of submissionNames){const record=JSON.parse(await kv.get(item.name||item.key)||"null");if(record?.subjectId===subjectId)submissions.push({submissionId:record.submissionId,status:record.status,title:record.title,category:record.category,createdAt:record.createdAt,reviewedAt:record.reviewedAt,reviewNote:record.reviewNote||"",publishedTemplateId:record.publishedTemplateId||null,upgradeTemplateId:record.upgradeTemplateId||null,upgradeFromVersion:record.upgradeFromVersion||null,submissionType:record.upgradeTemplateId?"VERSION_UPGRADE":"NEW_TEMPLATE"})}for(const item of inquiryNames){const record=JSON.parse(await kv.get(item.name||item.key)||"null");if(record?.subjectId===subjectId)inquiries.push(record)}const repository=new EdgeOneTemplateRepository(kv),templateIds=[...new Set(submissions.filter(item=>item.status==="APPROVED"&&item.publishedTemplateId).map(item=>item.publishedTemplateId))],grantNames=(await kv.list({prefix:"te_dg_"})).keys||[],publishedWorks=[];for(const templateId of templateIds){const template=await repository.getTemplate(templateId);if(!template||template.creatorPublicId!==subject?.publicId)continue;const sharedWith=[];for(const grantItem of grantNames){const grant=JSON.parse(await kv.get(grantItem.name||grantItem.key)||"null");if(grant?.templateId!==templateId||grant.enabled===false||grant.revokedAt||grant.grantedBy!==`creator:${subject.publicId}`)continue;const target=JSON.parse(await kv.get(`subject_${grant.subjectId}`)||"null");if(target?.publicId)sharedWith.push({publicId:target.publicId,grantedAt:grant.grantedAt})}publishedWorks.push({templateId,name:template.name,description:template.description||"",category:template.category,latestVersion:Number(template.latestVersion)||1,updatedAt:template.updatedAt||template.publishedAt||0,creatorSharingEnabled:template.creatorSharingEnabled!==false,sharedWith})}return json({ok:true,user:{publicId:subject?.publicId},submissions:submissions.sort((a,b)=>b.createdAt-a.createdAt),publishedWorks,inquiries:inquiries.sort((a,b)=>Number(b.updatedAt||b.createdAt)-Number(a.updatedAt||a.createdAt)),feedbackRetentionDays:30},200,headers)}
