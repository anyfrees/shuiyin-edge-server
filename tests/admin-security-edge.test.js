...
test('signed migration preserves an Argon2 password and issues a CSRF-bound admin session', async () => {
  ...
  response=await invoke(handler,'/admin/v1/console/templates/tpl_publish_ok',{method:'DELETE',headers:mutationHeaders});assert.equal(response.status,200);const removalNotice=JSON.parse(await kv.get('creator:notice:template-removed:tpl_publish_ok:sub_test'));assert.equal(removalNotice.type,'TEMPLATE_REMOVED');assert.equal(removalNotice.subjectId,'sub_test')
