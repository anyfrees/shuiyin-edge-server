...
test("EdgeOne subject entitlement grant revoke dependency batch and audit",async()=>{
  ...
  assert.deepEqual(await service.projection("a"),{workLogV1:false,workLogExportV1:false});
  ...
  assert.deepEqual(await service.projection("a"),{workLogV1:true,workLogExportV1:true});
