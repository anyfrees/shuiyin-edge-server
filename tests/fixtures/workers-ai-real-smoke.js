...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const provider = new CloudflareWorkersAIRefinementProvider({ ai: env.AI, model: input.model, temperature: Number(input.temperature ?? 0.1), maxTokens: 500 });
      const result = await provider.refineWorkLog(input.request || input.facts);
